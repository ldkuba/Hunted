...
public class WallHorizontalTile extends Tile
{
	private static final int AP_COST = 0;

	public WallHorizontalTile(int type, int x, int y, Main app, GameContainer gc)
	{
	...
	public void chooseImage() {
		...
		{
		if (this.y == 0)  {
				tileImage = new Image("res/textures/tiles/wallHorizontal1.png");
			} else if (this.y == Main.currentLevel.getLvlHeight()-1) {
				tileImage = new Image("res/textures/tiles/wallHorizontal1.png");
			} else {
				if ((Main.currentLevel.getType(this.x-1, this.y) == 1) && ((Main.currentLevel.getType(this.x+1, this.y) == 0)))
				{
					//Wood Wall Grass
					tileImage = new Image("res/textures/tiles/wallHorizontal2.png");
